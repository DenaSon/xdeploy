...
namespace App\Domain\Application\Marzban\Https\ValueObjects;

use App\Domain\Application\Marzban\Exceptions\InvalidMarzbanDomainException;

final readonly class MarzbanDomain
{
    ...
    public static function from(string $input): self
    {
        if (preg_match('/[\x00-\x1F\x7F]/', $input) === 1) {
            throw InvalidMarzbanDomainException::make();
        }
    ...
    private static function isValid(string $domain): bool
    {
        ...
        return preg_match('/[a-z]/', $labels[array_key_last($labels)]) === 1;
    }
}
