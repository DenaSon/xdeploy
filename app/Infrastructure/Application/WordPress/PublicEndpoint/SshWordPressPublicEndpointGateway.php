...
final readonly class SshWordPressPublicEndpointGateway implements WordPressPublicEndpointGateway
{
    private const string SITE_KEY = 'wordpress';

    private const string UPSTREAM = '127.0.0.1:8080';
...
if [ ! -d "$app_dir" ]; then
    ...
    if [ "$layout_state" = 'supported' ] && {
        ! printf '%s\n' "$compose_services" | grep -Fxq 'database' ||
        ! printf '%s\n' "$compose_services" | grep -Fxq 'wordpress';
    }; then
...
printf 'xdeploy_wordpress_server_preflight=1\n'
...
BASH;
    ...
    public function __construct(
        ...
        private SshWordPressPublicEndpointRuntimeManager $runtime,
    ...
    public function preflightServer(): PublicEndpointServerPreflightResult
    {
        ...
        if (($values['xdeploy_wordpress_server_preflight'] ?? null) !== '1') {
            throw PublicEndpointOperationException::preflightFailed();
        }
