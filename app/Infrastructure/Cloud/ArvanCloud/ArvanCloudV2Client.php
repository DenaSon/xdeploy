...
final class ArvanCloudV2Client
{
    ...
    private function request(
        ...
        try {
            $response = match ($method) {
                ...
                default => throw new CloudValidationException(
                    sprintf(
                        'Unsupported ArvanCloud v2 HTTP method [%s].',
                        $method,
                    ),
                ),
            };
    ...
    private function normalizeBaseUrl(
        ...
        if (
            ...
            throw new CloudConfigurationException(
                'ArvanCloud v2 base URL must be a valid HTTPS URL.',
            );
        }
    ...
    private function validateTimeout(
        ...
        if ($timeout < 1) {
            throw new CloudConfigurationException(
                sprintf(
                    'ArvanCloud v2 %s must be greater than zero.',
                    $name,
                ),
            );
        }
