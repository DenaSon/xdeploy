    ...
    @else
        <div class="space-y-5">
            <div class="grid gap-3 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-6">
                @foreach([
                    ['label' => 'بازدیدکننده', 'value' => $report['overview']['visitors'], 'meta' => 'کاربر یکتا در Landing', 'icon' => 'lucide.users'],
                    ['label' => 'احراز هویت', 'value' => $report['overview']['authenticated'], 'meta' => number_format($report['overview']['auth_conversion'], 1).'٪ تبدیل', 'icon' => 'lucide.log-in'],
                    ['label' => 'سفارش', 'value' => $report['overview']['orders'], 'meta' => number_format($report['overview']['order_conversion'], 1).'٪ از Buy', 'icon' => 'lucide.receipt-text'],
                    ['label' => 'پرداخت موفق', 'value' => $report['overview']['payments'], 'meta' => number_format($report['overview']['payment_conversion'], 1).'٪ از شروع پرداخت', 'icon' => 'lucide.badge-check'],
                    ['label' => 'سرور آماده', 'value' => $report['overview']['server_ready'], 'meta' => number_format($report['overview']['server_ready_rate'], 1).'٪ از پرداخت موفق', 'icon' => 'lucide.server'],
                    ['label' => 'فعال‌سازی', 'value' => $report['overview']['activated'], 'meta' => number_format($report['overview']['activation_rate'], 1).'٪ از سرور آماده', 'icon' => 'lucide.rocket'],
                ] as $metric)
