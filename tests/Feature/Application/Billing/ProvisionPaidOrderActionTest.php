...
final class ProvisionPaidOrderActionTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        config()->set('cloud.providers.arvan.defaults.network_id', 'network-default');
        config()->set('cloud.providers.arvan.defaults.security_group_id', 'security-default');
    }
    ...
    public function test_it_rejects_order_that_is_not_paid(): void
    {
        ...
        try {
            ...
            $this->assertNull($order->fresh()->server_id);
        }
    }
    ...
    public function test_fulfilled_order_returns_the_same_server_idempotently(): void
    {
        ...
        $this->assertSame($server->id, $result->id);
    ...
    public function test_failed_order_with_delivered_inactive_server_is_recovered_as_fulfilled_without_reprovisioning(): void
    {
        ...
        $server->forceFill(['host' => '203.0.113.77'])->save();
        $order->forceFill(['server_id' => $server->id])->save();
    ...
    public function test_failed_order_with_host_but_without_credential_is_not_marked_fulfilled(): void
    {
        $order = $this->order(OrderStatus::Failed);
        $server = $this->server(
            user: $order->user,
        ...
        $server->forceFill(['host' => '203.0.113.88'])->save();
        $order->forceFill(['server_id' => $server->id])->save();
        ...
        try {
            ...
            $freshOrder = $order->fresh();
            $this->assertSame(OrderStatus::Failed, $freshOrder->status);
            $this->assertSame($server->id, $freshOrder->server_id);
            $this->assertFalse($server->fresh()->hasCredential());
            $this->assertDatabaseCount('servers', 1);
        }
    }
    ...
    private function server(
        ...
        string $name,
        ServerStatus $status,
        ?string $credential = 'temporary-test-password',
    ): Server {
